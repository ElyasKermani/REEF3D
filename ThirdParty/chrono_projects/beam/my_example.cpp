...
int main(int argc, char* argv[]) {
    ...
    auto nodeC = chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(ChVector<>(1, 0, 0)));
    nodeA->SetMass(0.0);
    nodeB->SetMass(0.0);
    nodeC->SetMass(0.0);
    mesh->AddNode(nodeA);
    ...
    elementA->SetNodes(nodeA, nodeC);
    elementA->SetSection(msection);
    mesh->AddElement(elementA);

    // Create an Eulero-Bernoulli beam with a single element
    auto elementB = chrono_types::make_shared<ChElementBeamEuler>();
    elementB->SetNodes(nodeC, nodeB);
    elementB->SetSection(msection);
    mesh->AddElement(elementB);

    ChTriangleMeshConnected::WriteWavefront("Chrono_beam.obj", mesh);
    ...
    auto constrB = chrono_types::make_shared<ChLinkMateGeneric>();
    constrB->Initialize(nodeB, ground, false, nodeB->Frame(), nodeB->Frame());
    sys.Add(constrB);
    constrB->SetConstrainedCoords(true, true, true,   // x, y, z
                                  true, true, true);  // Rx, Ry, Rz
...
////#define LOAD_6
    ...
    mwrenchdis->loader.SetForcePerUnit(ChVector<>(0, -4000.0, 0));  // load per unit length
    loadcontainer->Add(mwrenchdis);

    auto mwrenchdis1 = chrono_types::make_shared<ChLoadBeamWrenchDistributed>(elementB);
    mwrenchdis1->loader.SetForcePerUnit(ChVector<>(0, -400.0, 0));  // load per unit length
    loadcontainer->Add(mwrenchdis1);
