// =============================================================================
//
// Chrono FEA using corotational hexa elements in the horizontal x-z plane  - modified version of the horizontal ANCF plate
//
// =============================================================================
...
int main(int argc, char* argv[]) {
    ...
    GetLog() << "---------------------------------------------------------------------------\n";
    GetLog() << "   Hexa CR Elements with horizontal placement and implicit integration   \n";
    GetLog() << "---------------------------------------------------------------------------\n";
    ...
    double plate_lenght_z = 1;  // small thickness
    ...
    */
    ...
    for (int i = 0; i < TotalNumElements; i++) {
        ...
        int iz = i / (numDiv_x * numDiv_y);

        NumNodes(i, 0) = (iz * (numDiv_y + 1) + iy) * (numDiv_x + 1) + ix;
        NumNodes(i, 1) = NumNodes(i, 0) + 1;
        NumNodes(i, 2) = NumNodes(i, 0) + numDiv_x + 2;
        NumNodes(i, 3) = NumNodes(i, 0) + numDiv_x + 1;
        NumNodes(i, 4) = NumNodes(i, 0) + (numDiv_x + 1) * (numDiv_y + 1);
        NumNodes(i, 5) = NumNodes(i, 4) + 1;
        NumNodes(i, 6) = NumNodes(i, 4) + numDiv_x + 2;
        NumNodes(i, 7) = NumNodes(i, 4) + numDiv_x + 1;
    ...
    for (int i = 0; i < TotalNumNodes; i++) {
        ...
        int iz = i / ((numDiv_x + 1) * (numDiv_y + 1));

        COORDFlex(i, 0) = ix * dx; // x-coordinate
        COORDFlex(i, 1) = iy * dy; // y-coordinate
        COORDFlex(i, 2) = iz * dz; // z-coordinate
    ...
    while (elemcount < TotalNumElements) {
        auto element = chrono_types::make_shared<ChElementHexaCorot_8>();
