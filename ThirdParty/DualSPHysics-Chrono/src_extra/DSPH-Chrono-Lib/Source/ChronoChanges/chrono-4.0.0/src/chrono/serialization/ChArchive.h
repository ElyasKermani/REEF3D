...
public:
      ...
      virtual int GetClassRegisteredVersion() {
		  ...
		  return class_factory::ChClassVersion<TClass>::version;  //<vs_dsphchanges> : Delete ambiguous chrono namespace
        }
      ...
      void VersionWrite() {
          ...
          if (this->cluster_class_versions) {
              if (this->class_versions.find(std::type_index(typeid(T))) == this->class_versions.end()) {
                this->out_version(class_factory::ChClassVersion<T>::version, typeid(T));
                this->class_versions[std::type_index(typeid(T))] = class_factory::ChClassVersion<T>::version;
              } 
          } 
          else {
              this->out_version(class_factory::ChClassVersion<T>::version, typeid(T));
