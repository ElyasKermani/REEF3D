...
export name=MutliBodyCollision_12R
...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartCube -onlymk:21-86
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/floatinginfo
${floatinginfo} -dirin ${diroutdata} -onlymk:21-86 -savedata ${dirout2}/FloatingMotion 
if [ $? -ne 0 ] ; then fail; fi
