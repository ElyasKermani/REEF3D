...
export name=MutliBodyCollision_6R
...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartCube -onlymk:21-53
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/floatinginfo
${floatinginfo} -dirin ${diroutdata} -onlymk:21-53 -savedata ${dirout2}/FloatingMotion 
if [ $? -ne 0 ] ; then fail; fi
