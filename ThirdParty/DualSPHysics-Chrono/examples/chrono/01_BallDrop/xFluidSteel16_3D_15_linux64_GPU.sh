...
export name=FluidSteel16_3D_15
...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartBall -onlytype:-all,+floating
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/floatinginfo
${floatinginfo} -dirin ${diroutdata} -onlymk:20 -savedata ${dirout2}/FloatingMotion 
if [ $? -ne 0 ] ; then fail; fi
