  ...
  ,const tfloat3 *boundnormal,const tfloat3 *motionvel,tfloat4 *velrhop)
{
  ...
  unsigned n=NpbOk;
  if(UseNormalsFt)n=Np; //-It does not work when fluid normal is not null. //<vs_fttmdbc>
...
//==============================================================================
void JSphCpu::ComputeStepMdbcLimit(const typecode *code,const tdouble3 *pospre
  ,tdouble3 *mov,tfloat4 *velrhop)const
{
  const tfloat3 limitmin=MdbcLimitMin;
  const tfloat3 limitmax=MdbcLimitMax;
  const bool chky=(limitmin.y>-FLT_MAX || limitmax.y<FLT_MAX);
  const bool chkzmin=(limitmin.z>-FLT_MAX);
  const bool chkrest=(limitmin.x>-FLT_MAX || limitmax.x<FLT_MAX || limitmax.z<FLT_MAX);
  const int np=int(Np);
  const int npb=int(Npb);
  ...
  for(int p=Npb;p<np;p++){
    ...
    if(CODE_IsFluidNotInout(rcode) && CODE_IsNormal(rcode)){
      const tdouble3 ps=pospre[p];
      const tdouble3 mv=mov[p];
      if(chkrest || chky){
        if(chkrest){
          if(ps.x+mv.x<=limitmin.x){
            mov[p].x=0;
            velrhop[p].x=fabs(velrhop[p].x);
          }
          if(ps.x+mv.x>=limitmax.x){
            mov[p].x=0;
            velrhop[p].x=-fabs(velrhop[p].x);
          }
        }
        if(chky){
          if(ps.y+mv.y<=limitmin.y){
            mov[p].y=0;
            velrhop[p].y=fabs(velrhop[p].y);
          }
          if(ps.y+mv.y>=limitmax.y){
            mov[p].y=0;
            velrhop[p].y=-fabs(velrhop[p].y);
          }
        }
      ...
      if(chkrest || chkzmin){
        if(chkzmin && ps.z<=limitmin.z){
          mov[p].z=0;
          velrhop[p].z=fabs(velrhop[p].z);
        }
        if(chkrest && ps.z>=limitmax.z){
          mov[p].z=0;
          velrhop[p].z=-fabs(velrhop[p].z);
        }
      }
...
void JSphCpu::ComputeSymplecticPre(double dt){
  ...
  //-Avoid penetration in a box domain.                          //<vs_fttmdbc>
  if(MdbcLimit)ComputeStepMdbcLimit(Codec,PosPrec,movc,Velrhopc);//<vs_fttmdbc>
