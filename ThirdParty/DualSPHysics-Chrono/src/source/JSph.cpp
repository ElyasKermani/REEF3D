...
void JSph::InitVars(){
  ...
  UseNormalsFt=false;
  SvNormals=false;
  MdbcLimit=false;                 //<vs_fttmdbc>
  MdbcLimitMin=TFloat3(-FLT_MAX);  //<vs_fttmdbc>
  MdbcLimitMax=TFloat3( FLT_MAX);  //<vs_fttmdbc>
  UseDEM=false;  //(DEM)
...
void JSph::LoadConfigCommands(const JSphCfgRun *cfg){
  ...
  if(TBoundary==BC_MDBC){
    ...
    {//<vs_fttmdbc_ini>
      ...
      for(unsigned cv=0;cv<nv;cv++)if(NuxLib->VarIsNum(cv)){
        const float v=float(NuxLib->GetVarNum(cv));
        const string vname=NuxLib->GetVarName(cv);
        if(vname=="MdbcLimitMinX")MdbcLimitMin.x=v;
        else if(vname=="MdbcLimitMinY")MdbcLimitMin.y=v;
        else if(vname=="MdbcLimitMinZ" || vname=="MdbcBottomLimitZ")MdbcLimitMin.z=v;
        else if(vname=="MdbcLimitMaxX")MdbcLimitMax.x=v;
        else if(vname=="MdbcLimitMaxY")MdbcLimitMax.y=v;
        else if(vname=="MdbcLimitMaxZ")MdbcLimitMax.z=v;
      }
      MdbcLimit=(MdbcLimitMin!=TFloat3(-FLT_MAX) || MdbcLimitMax!=TFloat3( FLT_MAX));
    }//<vs_fttmdbc_end>
  }
  ...
  if(MdbcLimit && TStep!=STEP_Symplectic)Run_Exceptioon("MdbcLimit option is only available for Symplectic."); //<vs_fttmdbc>
  ...
  ,const typecode *code,tfloat3 *boundnormal)
{
  ...
  if(!PartBegin){
    ...
    if(fun::FileExists(filenordata)){
      ...
      #ifdef DISABLE_MDBC_FT   //<vs_fttmdbc_ini>
        UseNormalsFt=false;
      #endif                   //<vs_fttmdbc_end>
      ...
      if(pnorsize){
        ...
        if(UseNormalsFt){ //<vs_fttmdbc_ini>
          if(pnorsize<CaseNbound)Run_ExceptioonFile("The number of final normals does not match boundary particles.",filenordata);
          for(unsigned p=npb;p<CaseNbound;p++)boundnormal[p]=ToTFloat3(pnor[p]);  //-For floating particles.
        } //<vs_fttmdbc_end>
  ...
  ,const unsigned *idp,tfloat3 *boundnormal)
{
  ...
  if(UseNormalsFt){
    ...
    if(!UseChrono || !ChronoObjects->GetUseCollision())
...
void JSph::VisuConfig(){
  ...
  if(TBoundary==BC_MDBC){
    ...
    if(MdbcLimit){ //<vs_fttmdbc_ini>
      if(MdbcLimitMin.x!=-FLT_MAX)Log->Print(fun::VarStr("  mDBC-LimitMinX",MdbcLimitMin.x));
      if(MdbcLimitMax.x!= FLT_MAX)Log->Print(fun::VarStr("  mDBC-LimitMaxX",MdbcLimitMax.x));
      if(MdbcLimitMin.y!=-FLT_MAX)Log->Print(fun::VarStr("  mDBC-LimitMinY",MdbcLimitMin.y));
      if(MdbcLimitMax.y!= FLT_MAX)Log->Print(fun::VarStr("  mDBC-LimitMaxY",MdbcLimitMax.y));
      if(MdbcLimitMin.z!=-FLT_MAX)Log->Print(fun::VarStr("  mDBC-LimitMinZ",MdbcLimitMin.z));
      if(MdbcLimitMax.z!= FLT_MAX)Log->Print(fun::VarStr("  mDBC-LimitMaxZ",MdbcLimitMax.z));
    }//<vs_fttmdbc_end>
    ...
    if(MdbcLimit){ //<vs_fttmdbc_ini>
      if(MdbcLimitMin.x!=-FLT_MAX)ConfigInfo=ConfigInfo+fun::PrintStr(" - mDBC-LimitMinX=%g",MdbcLimitMin.x);
      if(MdbcLimitMax.x!= FLT_MAX)ConfigInfo=ConfigInfo+fun::PrintStr(" - mDBC-LimitMaxX=%g",MdbcLimitMax.x);
      if(MdbcLimitMin.y!=-FLT_MAX)ConfigInfo=ConfigInfo+fun::PrintStr(" - mDBC-LimitMinY=%g",MdbcLimitMin.y);
      if(MdbcLimitMax.y!= FLT_MAX)ConfigInfo=ConfigInfo+fun::PrintStr(" - mDBC-LimitMaxY=%g",MdbcLimitMax.y);
      if(MdbcLimitMin.z!=-FLT_MAX)ConfigInfo=ConfigInfo+fun::PrintStr(" - mDBC-LimitMinZ=%g",MdbcLimitMin.z);
      if(MdbcLimitMax.z!= FLT_MAX)ConfigInfo=ConfigInfo+fun::PrintStr(" - mDBC-LimitMaxZ=%g",MdbcLimitMax.z);
    }//<vs_fttmdbc_end>
  ...
  ,const tdouble3 *pos,const unsigned *idp,const tfloat3 *boundnormal,float resize)const
{
  if(JVtkLib::Available()){
    ...
    //-Loads data of floating particles. //<vs_fttmdbc_ini>
    ...
    delete[] ftidx; ftidx=NULL;         //<vs_fttmdbc_end>
    //-Computes normalsize.
