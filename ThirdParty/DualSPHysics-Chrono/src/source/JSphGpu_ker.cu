...
//<vs_fttmdbc_ini>
//==============================================================================
/// Avoid penetration in some direction (in development).
//==============================================================================
template<bool chkzmin,bool chky,bool chkrest> __global__ void KerComputeStepMdbcLimit
  (unsigned n,unsigned pini,float3 limitmin,float3 limitmax
  ,const typecode *code,const double2 *posxypre,const double *poszpre
  ,double2 *movxy,double *movz,float4 *velrhop)
{
  ...
  if(pt<n){
    ...
    const typecode rcode=code[p];
    if(CODE_IsFluidNotInout(rcode) && CODE_IsNormal(rcode)){
      if(chkrest || chky){
        const double2 pxy=posxypre[p];
        const double2 mxy=movxy[p];
        if(chkrest){
          if(pxy.x+mxy.x<=limitmin.x){
            movxy[p].x=0;
            velrhop[p].x=fabs(velrhop[p].x);
          }
          if(pxy.x+mxy.x>=limitmax.x){
            movxy[p].x=0;
            velrhop[p].x=-fabs(velrhop[p].x);
          }
        }
        if(chky){
          if(pxy.y+mxy.y<=limitmin.y){
            movxy[p].y=0;
            velrhop[p].y=fabs(velrhop[p].y);
          }
          if(pxy.y+mxy.y>=limitmax.y){
            movxy[p].y=0;
            velrhop[p].y=-fabs(velrhop[p].y);
          }
        }
      ...
      if(chkrest || chkzmin){
        const double pz=poszpre[p]+movz[p];
        if(chkzmin && pz<=limitmin.z){
          movz[p]=0;
          velrhop[p].z=fabs(velrhop[p].z);
        }
        if(chkrest && pz>=limitmax.z){
          movz[p]=0;
          velrhop[p].z=-fabs(velrhop[p].z);
        }
      }
...
/// Avoid penetration in some direction (in development).
//==============================================================================
void ComputeStepMdbcLimit(unsigned np,unsigned npb,tfloat3 limitmin,tfloat3 limitmax
  ,const typecode *code,const double2 *posxypre,const double *poszpre
  ,double2 *movxy,double *movz,float4 *velrhop)
{
  const unsigned pini=npb;
  ...
  if(npf){
    dim3 sgrid=GetSimpleGridSize(npf,SPHBSIZE);
    const bool checky=(limitmin.y>-FLT_MAX || limitmax.y<FLT_MAX);
    const bool checkzmin=(limitmin.z>-FLT_MAX);
    const bool checkrest=(limitmin.x>-FLT_MAX || limitmax.x<FLT_MAX || limitmax.z<FLT_MAX);
    if(checkzmin){ const bool chkz=true;
      if(checky){ const bool chky=true;
        if(checkrest)KerComputeStepMdbcLimit<chkz,chky,true > <<<sgrid,SPHBSIZE>>> (npf,pini
          ,Float3(limitmin),Float3(limitmax),code,posxypre,poszpre,movxy,movz,velrhop);
        else         KerComputeStepMdbcLimit<chkz,chky,false> <<<sgrid,SPHBSIZE>>> (npf,pini
          ,Float3(limitmin),Float3(limitmax),code,posxypre,poszpre,movxy,movz,velrhop);
      }
      else{ const bool chky=false;
        if(checkrest)KerComputeStepMdbcLimit<chkz,chky,true > <<<sgrid,SPHBSIZE>>> (npf,pini
          ,Float3(limitmin),Float3(limitmax),code,posxypre,poszpre,movxy,movz,velrhop);
        else         KerComputeStepMdbcLimit<chkz,chky,false> <<<sgrid,SPHBSIZE>>> (npf,pini
          ,Float3(limitmin),Float3(limitmax),code,posxypre,poszpre,movxy,movz,velrhop);
