...
void sixdof_collision::calculate_collision_forces(lexer *p, ghostcell *pgc, vector<sixdof_obj*> &fb_obj)
{
    ...
    // Handle wall collisions after object-object collisions
    calculate_wall_collision_forces(p, pgc, fb_obj);
}

// New method to handle collisions between objects and domain walls
void sixdof_collision::calculate_wall_collision_forces(lexer *p, ghostcell *pgc, vector<sixdof_obj*> &fb_obj)
{
    // Get potential wall collisions from the grid
    std::vector<std::pair<int, int>> potential_wall_collisions = 
        collision_grid->find_potential_wall_collisions(p, pgc, fb_obj);
    
    if(p->mpirank==0 && p->count%p->P12==0 && potential_wall_collisions.size() > 0)
    {
        cout<<"6DOF Wall Collision: Found "<<potential_wall_collisions.size()<<" potential wall collisions"<<endl;
    }
    
    // Check each potential wall collision
    for(const auto& pair : potential_wall_collisions)
    {
        int obj_idx = pair.first;
        int wall_idx = pair.second;
        
        // Variables to store collision information
        Eigen::Vector3d contact_point, normal;
        double overlap = 0.0;
        
        // Detect if wall collision occurred
        if(detect_wall_collision(p, pgc, fb_obj[obj_idx], 
                               collision_grid->boundary_walls[wall_idx],
                               contact_point, normal, overlap))
        {
            // Calculate wall contact forces and torques
            Eigen::Vector3d force, torque;
            
            // Currently only linear model is implemented for wall collisions
            calculate_wall_linear_contact_force(p, pgc, fb_obj[obj_idx], 
                                              collision_grid->boundary_walls[wall_idx],
                                              contact_point, normal, overlap,
                                              force, torque);
            
            // Add wall collision forces to object
            fb_obj[obj_idx]->Xext += force(0);
            fb_obj[obj_idx]->Yext += force(1);
            fb_obj[obj_idx]->Zext += force(2);
            
            fb_obj[obj_idx]->Kext += torque(0);
            fb_obj[obj_idx]->Mext += torque(1);
            fb_obj[obj_idx]->Next += torque(2);
            
            if(p->mpirank==0 && p->count%p->P12==0)
            {
                cout<<"6DOF Wall Collision detected for object "<<obj_idx<<" with wall "<<wall_idx<<endl;
                cout<<"  Wall normal: ["<<normal(0)<<", "<<normal(1)<<", "<<normal(2)<<"]"<<endl;
                cout<<"  Overlap: "<<overlap<<endl;
                cout<<"  Force: ["<<force(0)<<", "<<force(1)<<", "<<force(2)<<"]"<<endl;
            }
        }
...
bool sixdof_collision::detect_wall_collision(lexer *p, ghostcell *pgc, sixdof_obj *obj, 
                                          ...
                                          double &overlap)
{
    ...
    Eigen::Vector3d center = obj->c_;
    double radius = obj->radius;
    
    // Calculate vector from wall reference point to object center
    Eigen::Vector3d wall_to_center = center - wall.reference_point;
    
    // Calculate signed distance from center to wall (positive if inside domain)
    double signed_distance = wall_to_center.dot(wall.normal_vector);
    
    // Check if object is overlapping with wall
    if(signed_distance < radius)
    {
        // Calculate overlap
        overlap = radius - signed_distance;
        
        // Wall normal points into the domain
        normal = wall.normal_vector;
        
        // Calculate contact point on wall
        contact_point = center - signed_distance * normal;
...
void sixdof_collision::calculate_wall_linear_contact_force(lexer *p, ghostcell *pgc, 
                                                        sixdof_obj *obj,
                                                        const boundary_wall_6DOF &wall,
...
{
    // Get object velocity at contact point
    Eigen::Vector3d obj_velocity = Eigen::Vector3d(obj->p_(0)/obj->Mass_fb, obj->p_(1)/obj->Mass_fb, obj->p_(2)/obj->Mass_fb);
    Eigen::Vector3d obj_angular_velocity = obj->omega_I;
    Eigen::Vector3d r = contact_point - obj->c_;
    Eigen::Vector3d contact_velocity = obj_velocity + obj_angular_velocity.cross(r);
    
    // Decompose velocity into normal and tangential components
    double normal_velocity = contact_velocity.dot(normal);
    Eigen::Vector3d tangential_velocity = contact_velocity - normal_velocity * normal;
    
    // Calculate normal force (spring-dashpot model)
    double normal_force_magnitude = spring_constant_n * overlap + 
                                   damping_constant_n * normal_velocity;
    
    // Ensure normal force is repulsive
    normal_force_magnitude = std::max(0.0, normal_force_magnitude);
    
    // Calculate normal force vector
    Eigen::Vector3d normal_force = normal_force_magnitude * normal;
    
    // Calculate tangential force (friction)
    Eigen::Vector3d tangential_force;
    
    if(tangential_velocity.norm() > 1.0e-10)
    {
        Eigen::Vector3d tangential_direction = tangential_velocity.normalized();
        double tangential_force_magnitude = std::min(
            wall.friction_coefficient * normal_force_magnitude,  // Coulomb limit
            spring_constant_t * overlap                         // Tangential spring
        );
        
        tangential_force = -tangential_force_magnitude * tangential_direction;
    }
    else
    {
